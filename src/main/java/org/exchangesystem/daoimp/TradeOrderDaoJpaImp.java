...
public class TradeOrderDaoJpaImp extends AbstractJpaDao<TradeOrder> implements
	...
	/****
	 * Get open orders for the type specified - SELL, BUY for the specified
	 * status OPEN for the symbol specied, the currency in question
	 ***/
	@Transactional
	public List<TradeOrder> findOpenBuyOrders(Symbol symbol,
		...
		Query q = em
				.createQuery(" from TradeOrder torder WHERE ((torder.symbol = :symbol) AND (torder.orderType = :orderType) AND (torder.orderStatus = :orderStatus)) ORDER BY price DESC ,  id ASC ");// .getResultList();
	...
	public List<TradeOrder> findOpenSellOrders(Symbol symbol,
		...
		EntityManager em = super.entityManager;
		Query q = em
				.createQuery(" from TradeOrder torder WHERE ((torder.symbol = :symbol) AND (torder.orderType = :orderType) AND (torder.orderStatus = :orderStatus) AND (torder.price <= :price) AND (torder.quantity = :quantity)) ORDER BY price ASC ,  id ASC ");// .getResultList();
		q.setParameter("symbol", symbol);
		q.setParameter("orderType", orderType);
		q.setParameter("orderStatus", orderStatus);
	...
	/***
	 * Get open buy orders - all that have not been processed so that the
	...
	public List<TradeOrder> findOpenBuyOrders(OrderType orderType,
		...
		EntityManager em = super.entityManager;
		Query q = em
				.createQuery(" from TradeOrder torder WHERE ((torder.orderType = :orderType) AND (torder.orderStatus = :orderStatus)) ORDER BY price DESC ,  id ASC ");// .getResultList();
		q.setParameter("orderType", orderType);
		q.setParameter("orderStatus", orderStatus);
	...
	public List<TradeOrder> findAll(OrderStatus orderStatus) {
		EntityManager em = super.entityManager;
		Query q = em
				.createQuery(" from TradeOrder torder WHERE ((torder.orderStatus = :orderStatus)) ORDER BY  id DESC ");// .getResultList();
		q.setParameter("orderStatus", orderStatus);
	...
	public List<TradeOrder> findOpenSellOrdersNewestFirst(Symbol symbol,
		...
		EntityManager em = super.entityManager;
		Query q = em
				.createQuery(" from TradeOrder torder WHERE ((torder.symbol = :symbol) AND (torder.orderType = :orderType) AND (torder.orderStatus = :orderStatus)) ORDER BY  id DESC ");// .getResultList();
		q.setParameter("symbol", symbol);
		q.setParameter("orderType", orderType);
		q.setParameter("orderStatus", orderStatus);
	...
	public List<TradeOrder> findOpenSellOrders(Symbol symbol,
		...
		EntityManager em = super.entityManager;
		Query q = em
				.createQuery(" from TradeOrder torder WHERE ((torder.symbol = :symbol) AND (torder.orderType = :orderType) AND (torder.orderStatus = :orderStatus) AND (torder.price <= :price)) ORDER BY price ASC ,  id ASC ");// .getResultList();
		q.setParameter("symbol", symbol);
		q.setParameter("orderType", orderType);
		q.setParameter("orderStatus", orderStatus);
		q.setParameter("price", price);
	...
	public List<TradeOrder> findOpenAndPartialBuyOrders(Symbol symbol,
		...
		EntityManager em = super.entityManager;
		Query q = em
				.createQuery(" from TradeOrder torder WHERE ((torder.symbol = :symbol) AND (torder.orderType = :orderType) AND ((torder.orderStatus = :orderStatusOpen) OR (torder.orderStatus = :orderStatusPartial))) ORDER BY price DESC ,  id ASC ");// .getResultList();
		q.setParameter("symbol", symbol);
		q.setParameter("orderType", orderType);
	...
	public List<TradeOrder> findOpenAndPartialSellOrders(Symbol symbol,
		...
		EntityManager em = super.entityManager;
		Query q = em
				.createQuery(" from TradeOrder torder WHERE ((torder.symbol = :symbol) AND (torder.orderType = :orderType) AND ((torder.orderStatus = :orderStatusOpen) OR (torder.orderStatus = :orderStatusPartial)) AND (torder.price <= :price)) ORDER BY price ASC ,  id ASC ");// .getResultList();
		q.setParameter("symbol", symbol);
		q.setParameter("orderType", orderType);
	...
	public List<TradeOrder> findOpenPartialBuyOrdersNewestFirst(Symbol symbol,
		...
		EntityManager em = super.entityManager;
		Query q = em
				.createQuery(" from TradeOrder torder WHERE ((torder.symbol = :symbol) AND (torder.orderType = :orderType) AND ((torder.orderStatus = :orderStatusOpen) OR (torder.orderStatus = :orderStatusPartial))) ORDER BY  id DESC ");// .getResultList();
		q.setParameter("symbol", symbol);
		q.setParameter("orderType", orderType);
		q.setParameter("orderStatusOpen", orderStatusOpen);
		q.setParameter("orderStatusPartial", orderStatusPartial);
	...
	public List<TradeOrder> findAllUnclosed(OrderStatus orderStatus,
		...
		EntityManager em = super.entityManager;
		Query q = em
				.createQuery(" from TradeOrder torder WHERE ((torder.orderStatus <> :orderStatus) AND (torder.createdBy = :exchangeUser)) ORDER BY  id DESC ");// .getResultList();
		q.setParameter("orderStatus", orderStatus);
